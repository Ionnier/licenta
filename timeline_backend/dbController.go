...
func syncUser(originalDb *sql.DB, user string) error {
	...
	if err := syncDb(originalDb, file, user); err == nil {
		if data, err := getUserInfo(user); err != nil {
			...
			if _, err := originalDb.ExecContext(context.TODO(), "insert into persons values(?, ?, ?, ?)", user, data.Data.UserName, data.Data.UserEmail, time.Now().UTC().Unix()); err != nil {
				log.Print(err)
...
func getUserInfo(user string) (AutoGenerated, error) {
	...
	req := a.Request()
	req.Header.SetMethod(fiber.MethodGet)
	req.SetRequestURI(fmt.Sprintf("%v/api/users/%v", os.Getenv(" "), user))
