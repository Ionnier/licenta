...
func syncUser(originalDb *sql.DB, user string) error {
	...
	if err := syncDb(originalDb, file, user); err == nil {
		if data, err := getUserInfo(user); err != nil {
			...
			if res, err := originalDb.ExecContext(context.TODO(), fmt.Sprintf("insert into persons values('%v', '%v', '%v', %v)", user, data.Data.UserName, data.Data.UserEmail, time.Now().UTC().Unix())); err != nil {
				log.Print(err)
